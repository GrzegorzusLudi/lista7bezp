...
void runsocket(int port,string sd,string se,string sn){
    ...
    while(true){
        ...
        while(read( new_socket , buffer, keysize)){
            mpz_t message,d,n,e,f,random,gcda,randome;
            gmp_randstate_t rs;
            ...
            mpz_powm(randome,random,e,n);

            mpz_mul(randome,message,randome);
            mpz_mod(randome,randome,n);

            mpz_invert(random,random,n);

            mpz_powm(randome,randome,d,n);

            mpz_mul(randome,randome,random);
            mpz_mod(randome,randome,n);

            gmp_sprintf (buffer2,"%Zx", randome);
            //gmp_printf ("%Zx\n", f);
            //printf("%s",buffer2 );
